...
public:
  ...
  const Student *searchByRoll(int roll)
  const {
    for (int i = 0; i < count; i++)
    {
...
int main()
{
  ...
  Student s3("Vikram", 103, 80);

  manager.addStudent(s1);
  manager.addStudent(s2);
  manager.addStudent(s3);

  cout << "/n All Students Record";

  manager.displayAll();

  cout << "/n Searching Students Record via Rollno";

  const Student* found = manager.searchByRoll(102);
