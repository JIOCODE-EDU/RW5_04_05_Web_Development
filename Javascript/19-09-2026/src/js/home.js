...
const renderProducts = (list) => {
  ...
  container.innerHTML = "";
  container.className="flex flex-wrap justify-evenly gap-6"
  ...
  list.forEach((product) => {
    ...
    card.className = "";
    card.innerHTML = `
<div class="w-80 bg-neutral-primary-soft p-6 border border-default rounded-base shadow-xs">
    <a href="#">
        <img class="rounded-base mb-6 h-28 w-32 mx-auto" src="${product.image}" alt="product image" />
    </a>
    <div>
        ...
        <a href="#">
            <h5 class="text-xl text-heading font-semibold tracking-tight truncate">${product.title}</h5>
        </a>
        <div class="flex items-center justify-between mt-6">
            <span class="text-3xl font-extrabold text-heading">$${product.price}</span>
            <button type="button" class="inline-flex items-center bg-brand hover:bg-brand-strong box-border border focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-2 py-2 focus:outline-none add-cart-btn cursor-pointer">
...
const addToCart = (productId) => {
  ...
  const existingItem = cart.find((p) => p.id === productId.id)
